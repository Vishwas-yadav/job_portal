...
const setCandVerifiedService=async (params)=>{
    try {
        let {cand_id,setVerified}=params;
        if(!cand_id){
            throw {
                br: true,
                msg: {
        ...
        const user=await candidate.findOneAndUpdate(filter,{isVerified:setVerified});
        if(!user){
            throw {
                br: true,
                msg: {
        ...
        console.log("User found::", user);
        let result={
            msg:"Candidate Access Updated successfully!"
        }
        return result;
    } catch (error) {
        console.log("Error in 'setCandVerifiedService' Service:", error);
        throw error;
    }
    }
